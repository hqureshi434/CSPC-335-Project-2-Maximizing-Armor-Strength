...
// followed by the total cost and defense in it.
...
{	
	...
	for (auto& armor : source){
		if (armor->defense() >= min_defense &&
			armor->defense() <= max_defense &&
			filtered->size() != total_size)
...
)
{
	...
	//bubble sort algorithm. highest defense per cost to lowest defense per cost.
	for (int i = 0; i < todo.size()-1; i++){
		for(int j = 0; j < todo.size()-i-1; j++){
			if ((todo[j]->defense() / todo[j]->cost()) < (todo[j+1]->defense() / todo[j+1]->cost())){ // as shown in class, defense / cost
				std::swap(todo[j],todo[j+1]);                                                        // sort from high to low. 
			}
		}
	}
	// vector sorted. now we can check from the top of vector everytime until vector is empty.
	while(!todo.empty()){  
		std::shared_ptr<ArmorItem> a = todo[0]; //use shared pointer "a" to assign to item. Max def/cost item will always be on top.
		float g = a->cost();	  // store item cost into variable "g"
		todo.erase(todo.begin()); // remove "a" from todo
		if (result_cost + g <= total_cost){  // check if item can be afforded
			result->push_back(a);  // if can be afforded, add item to result vector
			result_cost += g;     // amount of gold spent accumulates
		}
	}
	return result;
}
...
)
{
	...
	assert(n < 64);
	double total_cost_cand, total_defense_cand;                // declare variables for summing totals
	double total_cost_best, total_defense_best;
	
	std::unique_ptr<ArmorVector> best(new ArmorVector);        
	
	double end = pow(2,n);
	
	for(uint64_t bits = 0; bits < end; bits++){                     //algorithm provided by project instructions
		std::unique_ptr<ArmorVector> candidate(new ArmorVector);
		for(int j = 0; j < n; j++){
			if(((bits >> j) & 1) == 1)
				candidate->push_back(armors[j]);
		}
		sum_armor_vector(*candidate, total_cost_cand, total_defense_cand);    //stores totals into variables
		sum_armor_vector(*best, total_cost_best, total_defense_best);
		
		if(total_cost_cand <= total_cost){                                   //narrows down to the most optimal armor defense per cost
			if(best->empty() || total_defense_cand > total_defense_best)      
				*best = *candidate;
		}
	}
	return best;
